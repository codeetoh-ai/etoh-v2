from fastapi import APIRouter, HTTPException, Depends, status
from pydantic import BaseModel
from typing import Optional

from ..auth import get_current_admin
from ..database import supabase

router = APIRouter(prefix="/api/insights", tags=["insights"])

TABLE = "insights"


# ── Pydantic Models ──

class AuthorModel(BaseModel):
    name: str
    role: str = ""


class SidebarItemModel(BaseModel):
    label: str
    text: str


class SidebarModel(BaseModel):
    title: str = ""
    items: list[SidebarItemModel] = []
    ctaText: str = ""


class SectionModel(BaseModel):
    type: str  # paragraph | heading | bullets
    text: str = ""
    level: Optional[int] = None
    items: Optional[list[str]] = None


class InsightCreate(BaseModel):
    slug: str
    category: str
    date: str
    title: str
    insightType: str = "whitepaper"  # whitepaper | report | research_brief | case_study
    author: AuthorModel
    heroImage: str = ""
    quote: str = ""
    excerpt: str = ""
    sections: list[SectionModel] = []
    sidebar: SidebarModel = SidebarModel()
    relatedSlugs: list[str] = []


class InsightUpdate(BaseModel):
    category: Optional[str] = None
    date: Optional[str] = None
    title: Optional[str] = None
    insightType: Optional[str] = None
    author: Optional[AuthorModel] = None
    heroImage: Optional[str] = None
    quote: Optional[str] = None
    excerpt: Optional[str] = None
    sections: Optional[list[SectionModel]] = None
    sidebar: Optional[SidebarModel] = None
    relatedSlugs: Optional[list[str]] = None


# ── Helpers ──

def row_to_insight(row: dict) -> dict:
    """Convert a DB row to the frontend insight shape."""
    return {
        "id": row["id"],
        "slug": row["slug"],
        "category": row["category"],
        "date": row["date"],
        "title": row["title"],
        "insightType": row.get("insight_type", "whitepaper"),
        "author": {
            "name": row["author_name"],
            "role": row.get("author_role", ""),
        },
        "heroImage": row.get("hero_image", ""),
        "quote": row.get("quote", ""),
        "excerpt": row.get("excerpt", ""),
        "sections": row.get("sections", []),
        "sidebar": row.get("sidebar", {"title": "", "items": [], "ctaText": ""}),
        "relatedSlugs": row.get("related_slugs", []),
    }


def insight_to_row(data: dict) -> dict:
    """Convert frontend insight shape to DB row."""
    row = {
        "slug": data["slug"],
        "category": data["category"],
        "date": data["date"],
        "title": data["title"],
        "insight_type": data.get("insightType", "whitepaper"),
        "author_name": data["author"]["name"],
        "author_role": data["author"].get("role", ""),
        "hero_image": data.get("heroImage", ""),
        "quote": data.get("quote", ""),
        "excerpt": data.get("excerpt", ""),
        "sections": data.get("sections", []),
        "sidebar": data.get("sidebar", {}),
        "related_slugs": data.get("relatedSlugs", []),
    }
    return row


# ── GET /api/insights — public: list all insights ──

@router.get("")
def list_insights(insight_type: Optional[str] = None):
    query = supabase.table(TABLE).select("*")
    if insight_type:
        query = query.eq("insight_type", insight_type)
    res = query.order("created_at", desc=True).execute()
    return [row_to_insight(r) for r in res.data]


# ── GET /api/insights/{slug} — public: single insight ──

@router.get("/{slug}")
def get_insight(slug: str):
    res = supabase.table(TABLE).select("*").eq("slug", slug).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="Insight not found")
    return row_to_insight(res.data[0])


# ── POST /api/insights — admin: create insight ──

@router.post("", status_code=status.HTTP_201_CREATED)
def create_insight(body: InsightCreate, admin: dict = Depends(get_current_admin)):
    existing = supabase.table(TABLE).select("id").eq("slug", body.slug).execute()
    if existing.data:
        raise HTTPException(status_code=409, detail="An insight with this slug already exists")

    row = insight_to_row(body.model_dump())
    res = supabase.table(TABLE).insert(row).execute()
    return row_to_insight(res.data[0])


# ── PUT /api/insights/{slug} — admin: update insight ──

@router.put("/{slug}")
def update_insight(slug: str, body: InsightUpdate, admin: dict = Depends(get_current_admin)):
    existing = supabase.table(TABLE).select("id").eq("slug", slug).execute()
    if not existing.data:
        raise HTTPException(status_code=404, detail="Insight not found")

    updates = {}
    data = body.model_dump(exclude_none=True)

    if "author" in data:
        updates["author_name"] = data["author"]["name"]
        updates["author_role"] = data["author"].get("role", "")
        del data["author"]

    if "insightType" in data:
        updates["insight_type"] = data.pop("insightType")

    if "heroImage" in data:
        updates["hero_image"] = data.pop("heroImage")

    if "relatedSlugs" in data:
        updates["related_slugs"] = data.pop("relatedSlugs")

    if "sidebar" in data:
        updates["sidebar"] = data.pop("sidebar")

    if "sections" in data:
        updates["sections"] = data.pop("sections")

    updates.update(data)

    if not updates:
        raise HTTPException(status_code=400, detail="No fields to update")

    res = supabase.table(TABLE).update(updates).eq("slug", slug).execute()
    return row_to_insight(res.data[0])


# ── DELETE /api/insights/{slug} — admin: delete insight ──

@router.delete("/{slug}")
def delete_insight(slug: str, admin: dict = Depends(get_current_admin)):
    existing = supabase.table(TABLE).select("id").eq("slug", slug).execute()
    if not existing.data:
        raise HTTPException(status_code=404, detail="Insight not found")

    supabase.table(TABLE).delete().eq("slug", slug).execute()
    return {"message": "Insight deleted"}
