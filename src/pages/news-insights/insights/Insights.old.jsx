...
function TypewriterHeadline({ headline }) {
    const HEADLINE_LINES = headline.split('\n')
    const HEADLINE_FULL  = headline
...
export default function InsightsPage() {
    ...
    const { clinical, loading: clinicalLoading } = useClinical()
    
    // Default settings object
    const settings = {
        heroBadge: 'INSIGHTS & ANALYTICS',
        heroHeadline: 'System Intelligence\nAcross a National Network',
        sysIntelTitle: 'Operational Intelligence',
        sysIntelSubtitle: 'Real-time network analytics',
        metricValue: 94.2,
        ...
        <PageLayout title="" fullWidth={true} lightHero>
            ...
            <div style={{ paddingLeft: isMobile ? 16 : 24, paddingRight: isMobile ? 16 : 24, width: '100%', boxSizing: 'border-box' }}>
                ...
                <div style={{ paddingTop: isMobile ? 48 : 80, paddingBottom: isMobile ? 48 : 96, display: 'flex', flexDirection: 'column', gap: 24 }}>
                    ...
                    <div style={{ paddingBottom: 8 }}>
                        <h1 style={{ margin: 0, color: '#001736', fontSize: isMobile ? 22 : 72, fontFamily: 'Manrope', fontWeight: 800, lineHeight: isMobile ? '30px' : '72px', minHeight: isMobile ? 'auto' : '360px', wordBreak: 'break-word', overflowWrap: 'break-word', maxWidth: '100%' }}>
                            <TypewriterHeadline headline={settings.heroHeadline} />
                        </h1>
                    </div>
                ...
                <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 40 : 32, paddingBottom: isMobile ? 48 : 96 }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: isMobile ? 32 : 64 }}>
                                    ...
                                    >
                                        <span style={{ textAlign: 'right', color: '#001736', fontSize: isMobile ? 26 : 36, fontFamily: 'Manrope', fontWeight: 900, lineHeight: isMobile ? '32px' : '40px' }}>
                                            <CountUp end={settings.metricValue} duration={1.2} active={metricCardInView} decimals={1} suffix="%" />
                                        </span>
                    ...
                    {/* ── Right column (Clinical Perspectives) — HIDDEN ─────────────────────────────────── */}
                    <div ref={rightCardsRef} style={{ flex: 1, display: 'none', flexDirection: 'column', gap: 32 }}>
                ...
                {insights && insights.length > 0 && (
                    <div style={{ padding: isMobile ? '32px 16px 64px' : '48px 24px 80px', background: '#F8F9FA' }}>
                        <div ref={insightsGridRef} style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 40 }}>
                            <motion.div
